...
async def add_payment(payload: PaymentCreate, db: AsyncSession = Depends(get_db)):
    ...
    if order.payment_status == PaymentStatus.paid:     # ← lowercase
        raise HTTPException(409, "Order is already fully paid")

    balance = float(order.balance_amount)
    if payload.amount > balance + 0.01:
        raise HTTPException(422, f"Payment ₹{payload.amount} exceeds balance ₹{balance:.2f}")
    ...
    if order.balance_amount <= 0:
        order.payment_status = PaymentStatus.paid      # ← lowercase
    elif float(order.paid_amount) > 0:
        order.payment_status = PaymentStatus.partial   # ← lowercase
